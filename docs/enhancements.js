(() => {
  ...
  function employeeNumberLabel(employee) {
    return `<span class="employee-number-label"><small>Nr pracownika</small><b>${escapeHtml(employee.code)}</b></span>`;
  }
    ...
    legacy.innerHTML = `<header class="time-roster-head"><div><span class="kicker">KOMPAKTOWA LISTA OBECNOŚCI</span><h3>Pracownicy, godziny i przerwy</h3><p>Najpierw widzisz tylko podsumowanie. Kliknij wiersz pracownika, aby zmienić jego harmonogram.</p></div><div class="time-legend"><span><i class="green"></i>czas netto</span><span><i class="amber"></i>15 min płatne</span><span><i class="blue"></i>czas obecności</span></div></header>
      <div class="time-roster">${employees.map((employee) => {
        ...
        return `<details class="time-worker-card ${present ? "" : "inactive"}" data-time-card-id="${employee.id}" ${expanded ? "open" : ""}>
          <summary><span class="person"><i class="avatar">${employee.name.split(" ").map((part) => part[0]).join("").slice(0, 2)}</i><span>${employeeNameLink(employee)}<span class="employee-identity-meta">${employeeNumberLabel(employee)}${employeeLanguageChip(employee)}</span></span></span><span class="time-worker-summary"><em>${employee.status}</em><span class="time-range"><small>Godziny</small><b>${present ? `${employee.start}–${employee.end}` : "—"}</b></span><span class="time-break-summary"><small>Przerwy</small><b>${present ? `${accounting.total} min · ${accounting.paid} płatne` : "—"}</b></span><strong>${present ? `${formatMinutes(net)} netto` : "Brak godzin"}</strong><i class="time-chevron">⌄</i></span></summary>
  ...
  function enhanceTaskAssignmentForm() {
    ...
    if (form && !form.classList.contains("v5-smart-assignment")) {
      ...
      if (picker) {
        ...
        labels.forEach((label, index) => {
          ...
          const number = label.querySelector("span > small");
          if (employee && number) { number.classList.add("employee-number-inline"); number.textContent = `Nr pracownika: ${employee.code}`; }
    ...
    if (reassign && !reassign.classList.contains("v5-smart-reassign")) {
      ...
      Array.from(select?.options || []).forEach((option) => {
        ...
        const identity = employee ? `${employeeName} · Nr pracownika: ${employee.code} · ${employee.languageCode} ${employee.language}` : employeeName;
