(() => {
  ...
  const taskSeed = [
    { id: 1, title: "Zakładanie zawieszek", row: "R01", side: "Lewa", cart: "WZ-01", people: ["Chidi Eze", "Elmar Gabarimov", "Mariah Parker"], status: "W trakcie", unit: "rz.", progress: 68, contributions: [] },
    { id: 2, title: "Zakładanie zawieszek", row: "R01", side: "Prawa", cart: "WZ-02", people: ["Riley Tan", "Rachid Khudaverdov"], status: "Zakończone", unit: "rz.", result: 6, hours: 6, progress: 100, contributions: [{ person: "Riley Tan", result: 3, hours: 3 }, { person: "Rachid Khudaverdov", result: 3, hours: 3 }] },
    { id: 3, title: "Obcinanie liści", row: "R07", side: "Lewa", cart: "WZ-03", people: ["Anar Akhmedov", "Ismail Kuliev", "Maria Lutak"], status: "W trakcie", unit: "rz.", progress: 58, contributions: [] },
    { id: 4, title: "Zbiór", row: "R12", side: "Obie", cart: "WZ-07", people: ["Ali Sidyma"], status: "Zakończone", unit: "kg", result: 132, hours: 1, progress: 100, contributions: [{ person: "Ali Sidyma", result: 132, hours: 1 }] },
  ];
  const ticketSeed = [
  ...
  function individualResults() {
    return state.tasks.flatMap((task) => (task.contributions || []).map((entry) => ({ ...entry, title: task.title, row: task.row, cart: task.cart, unit: task.unit })));
  }
  ...
  function tasks() {
    return `${pageHead("PLAN I WYKONANIE", "Prace na zmianie", "Każde zadanie łączy ludzi, rząd i wózek, dzięki czemu wiadomo kto co robi i wykonał.", `<button class="primary" data-action="new-task">+ Dodaj zadanie</button>`)}
  ...
  function reports() {
    ...
    return `${pageHead("ANALIZA OPERACYJNA", "Raport zmiany", "System agreguje obecność, wykonane prace, wyniki i wyjątki.", `<button class="secondary" data-action="export-report">↓ Eksport JSON</button> <button class="primary" data-action="open-close">${state.shiftClosed ? "Zobacz zamknięcie" : "Zamknij zmianę"}</button>`)}
      ...
      <section class="metrics">${metric("Łączne godziny", totalHours.toFixed(2), "w widocznym okresie", "◷")}${metric("Aktywni pracownicy", presentCount(), "w brygadzie", "♙")}${metric("Wpisy osobowe", entries.length, "osoba + rząd + wózek", "✓")}${metric("Pozycje z uwagami", activeTickets(), "do weryfikacji", "!", "red")}</section>
      <section class="assignment-register surface"><div><span class="kicker">REJESTR WYKONANIA</span><h3>Kto wykonał pracę</h3></div>${entries.map((entry)=>`<div class="assignment-entry"><span class="person"><i class="avatar">${initials(entry.person)}</i><span><b>${entry.person}</b><small>${entry.title}</small></span></span><span><small>Rząd</small><b>${entry.row}</b></span><span><small>Wózek</small><b>${entry.cart}</b></span><span><small>Wynik</small><b>${entry.result} ${entry.unit}</b></span><span><small>Czas</small><b>${entry.hours} h</b></span></div>`).join("")}</section>
  ...
  app.addEventListener("submit", (event) => {
    ...
    if (form.dataset.form === "new-task") { const people=data.getAll("employees"); if(!people.length){notify("Wybierz co najmniej jedną osobę");return;} state.tasks.push({ id:Date.now(), title:data.get("title"), row:data.get("row"), side:data.get("side"), cart:data.get("cart"), people, status:"W trakcie", unit:data.get("unit"), progress:0, contributions:[] }); state.modal=null; notify(`Przydzielono ${people.length} os. do ${data.get("row")} · ${data.get("cart")}`); }
